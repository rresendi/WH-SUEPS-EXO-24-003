...
bool user::Execute(SampleFormat &sample, const EventFormat &event)
{
  ...
  // WpT / SUEPpT < 3
  double ratio = recoW.pt() / Ak15Jets.at(0).pt();
  bool W_SUEP_ratio = (ratio < W_SUEP_PT_RATIO);
  ...
  if (outFile.tellp() == 0) { 
    outFile << "EventNumber,ak15NTracks,wTransverseMass,wPt,ak4Pt,boostedSphericity,metPt,looseLep,tightLep,NJets,ak15Pt,wPhi,W_SUEP_ratio\n"; 
  }
